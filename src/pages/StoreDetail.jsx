...
const StoreDetail = () => {
  ...
  return (
    <StContainer>
      <StStore>
        <StStoreCol>
          <StoreImage src={data.image} alt={data.name} width='500' height='625'/>
        ...
        <StStoreInfo>
          ...
          <div>
            ...
            <StStoreInfoContent>{data.address}</StStoreInfoContent>
          </div>
          <div>
        ...
        <StaticMap lng={data.marker.x} lat={data.marker.y} title={data.address}/>
        </StStoreCol>
      </StStore>
      <StDivider/>
      <StCommentsContainer>
...
const StaticMap = ({lat, lng, title}) => {
  ...
  <StMapContainer>
    ...
    <StMapTitle><span>{title}</span></StMapTitle>
  </StMapContainer>
  )
}
